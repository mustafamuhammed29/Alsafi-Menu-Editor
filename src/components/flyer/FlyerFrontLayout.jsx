...
import { Upload, Flame, Crown, Sparkles, Phone, MapPin, QrCode, Tag, ShieldCheck } from 'lucide-react';
import { optimizeImageFile } from '../../utils/imageOptimizer';

export const FlyerFrontLayout = ({ flyerData, onUpdate }) => {
  const { updateFlyerFront } = useMenu();
  const heroImageInputRef = useRef(null);
  const [hoveredCover, setHoveredCover] = useState(false);

  const front = flyerData?.front || {};

  const heroImage = front.heroImage || 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?auto=format&fit=crop&w=1800&q=88';
  const heroScale = front.heroScale !== undefined ? front.heroScale : 1.05;
  const heroPosX = front.heroPosX !== undefined ? front.heroPosX : 50;
  const heroPosY = front.heroPosY !== undefined ? front.heroPosY : 50;
  const heroDarkness = front.heroDarkness !== undefined ? front.heroDarkness : 20;

  const badgeSize = front.badgeSize || 9.5;
  const titleSize = front.titleSize || 36;
  const subtitleSize = front.subtitleSize || 12;
  const taglineSize = front.taglineSize || 9.5;
  const logoSize = front.logoSize || 72;
  const footerValueSize = front.footerValueSize || 9.5;
  ...
  const handleHeroImageUpload = async (e) => {
    ...
    try {
      ...
      updateFlyerFront('heroImage', optimized);
    } catch {
      const reader = new FileReader();
      reader.onload = (ev) => {
        updateFlyerFront('heroImage', ev.target.result);
      };
  ...
  return (
    <div className="a4-page-wrapper" id="flyer-front">
      <div
      ...
      >
        {/* ─── LAYER 0: FULL BACKGROUND HERO FOOD PHOTO ──────────────────────────────── */}
        <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none z-0">
          <img
            src={heroImage}
            alt="Alsafi Flyer Front"
          ...
          {/* User-customizable darkness tint */}
          {heroDarkness > 0 && (
            <div
        ...
        {/* ─── LAYER 2: TOP IMPERIAL BRANDING & LOGO ────────────────────────────────── */}
        <header className="relative z-20 pt-7 px-8 flex flex-col items-center text-center shrink-0">
          ...
          <div className="mb-1 flex items-center justify-center gap-2">
            ...
            <div className="bg-black/70 border border-brand-gold/80 px-3.5 py-0.5 rounded-full backdrop-blur-md shadow-lg">
              <EditableText
                value={front.badge || 'ORIENTALISCHE GASTRONOMIE SEIT 2018 · HEIDELBERG'}
                onChange={(v) => updateFlyerFront('badge', v)}
          ...
          <div className="my-0.5">
            <EditableText
              value={front.title || 'ALSAFI RESTAURANT'}
              onChange={(v) => updateFlyerFront('title', v)}
              tagName="h1"
          ...
          <div>
            <EditableText
              value={front.subtitle || '✦ FEINSTE ORIENTALISCHE SPEZIALITÄTEN ✦'}
              onChange={(v) => updateFlyerFront('subtitle', v)}
          ...
          <div className="mt-1.5">
            <span className="inline-flex items-center px-4 py-0.5 rounded-full bg-black/70 border border-brand-gold/60 text-brand-gold font-cinzel font-bold tracking-widest uppercase shadow-lg backdrop-blur-sm">
              <EditableText
                value={front.tagline || 'SCHAWARMA · HOLZKOHLEGRILL · MEZZE · WRAPS'}
                onChange={(v) => updateFlyerFront('tagline', v)}
                className="block"
        ...
        {/* ─── LAYER 3: SPECIAL PROMO OFFER BADGE (شارة العرض الخاص البارزة) ──────────── */}
        {front.showOfferBadge !== false && (
          <div className="relative z-20 mx-auto my-2 shrink-0">
            <div className="bg-gradient-to-r from-red-950/90 via-red-900/95 to-red-950/90 border-2 border-yellow-400/90 rounded-2xl px-6 py-2.5 shadow-[0_8px_25px_rgba(220,38,38,0.5)] backdrop-blur-md text-center transform hover:scale-105 transition">
              <div className="flex items-center justify-center gap-2">
                ...
                <EditableText
                  value={front.offerBadgeTitle || '🔥 10% RABATT'}
                  onChange={(v) => updateFlyerFront('offerBadgeTitle', v)}
                  className="font-cinzel text-base font-black text-yellow-300 tracking-wider uppercase drop-shadow block"
                />
                <Tag className="w-4 h-4 text-yellow-300" />
              </div>
              <EditableText
                value={front.offerBadgeSubtitle || 'BEI ABHOLUNG & BARZAHLUNG'}
                onChange={(v) => updateFlyerFront('offerBadgeSubtitle', v)}
        ...
        {/* ─── LAYER 4: CENTER INTERACTIVE HOVER ACTIONS ─────────────────────────────── */}
        <div className="relative z-20 flex-1 flex flex-col items-center justify-center pointer-events-none">
          {hoveredCover && (
            <button
              ...
              title="تغيير أو رفع صورة الواجهة"
            >
              <Upload className="w-4 h-4 text-black" />
              <span>تغيير صورة واجهة الفلاير</span>
            </button>
          )}
        ...
        {/* ─── LAYER 5: 3 CULINARY PILLARS HIGHLIGHT CARDS ──────────────────────────── */}
        <div className="relative z-20 mx-7 my-2 shrink-0">
          <div className="grid grid-cols-3 gap-2.5">
            <div className="bg-black/75 border border-brand-gold/60 rounded-xl p-2.5 text-center backdrop-blur-md shadow-xl">
              ...
              <div className="text-[7.5px] text-gray-300 font-medium mt-0.5 leading-tight">
                Echtes Raucharoma & frische Spieße
              </div>
            </div>
            ...
            <div className="bg-black/75 border border-brand-gold/60 rounded-xl p-2.5 text-center backdrop-blur-md shadow-xl">
              ...
              <div className="text-[7.5px] text-gray-300 font-medium mt-0.5 leading-tight">
                Täglich frisch gesteckt nach Hausrezept
              </div>
            </div>
            ...
            <div className="bg-black/75 border border-brand-gold/60 rounded-xl p-2.5 text-center backdrop-blur-md shadow-xl">
              ...
              <div className="text-[7.5px] text-gray-300 font-medium mt-0.5 leading-tight">
                Knusprige Falafel & feine Mezze Dips
              </div>
            </div>
          </div>
        </div>

        {/* ─── LAYER 6: BOTTOM FOOTER BAR & QUICK QR CODE ───────────────────────────── */}
        <footer className="relative z-20 mx-7 mb-5 p-3 rounded-2xl bg-[#03140a]/95 border-2 border-brand-gold/70 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.95)] shrink-0 flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-white font-mono leading-tight" style={{ fontSize: `${footerValueSize}px` }}>
                <EditableText
                  value={front.phone || '06221 72 59 000'}
                  onChange={(v) => updateFlyerFront('phone', v)}
                />
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-gray-200 leading-tight" style={{ fontSize: `${footerValueSize}px` }}>
                <EditableText
                  value={front.address || 'Hertzstraße 1, 69126 Heidelberg (Kaufland)'}
                  onChange={(v) => updateFlyerFront('address', v)}
                />
              </span>
            </div>
          </div>

          {/* Quick QR Code for Online Menu */}
          <div className="flex items-center gap-2 bg-black/60 border border-brand-gold/50 rounded-xl px-3 py-1.5 shadow">
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(front.qrUrl || 'https://alsafi-restaurant.de')}`}
              alt="QR Code"
...
export default FlyerFrontLayout;
